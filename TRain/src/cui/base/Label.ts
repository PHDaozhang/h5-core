...
module cui
{
    export class Label extends egret.TextField implements IBaseCtrl
    {
        public tag:number;
        public ud:any;
        ...
        public set txtFlowKey(key:string){
            let txt = TRain.langMgr.getTxtByKey( key );
            if( txt ) this.textFlow =  htmlParser.parser(txt);
        }
        ...
        public validateProps():void
        {
            ...
            if( invalidateProps != 0 )
            {
                ...
                {
                    if( self.$TextField[egret.sys.TextKeys.text] != "" )
                    {
                        (<BaseContainer><any>self.$parent).invalidateDL();
                    }
                }
